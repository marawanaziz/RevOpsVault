---
date: {{date}}
client: {{client}}
severity: low | medium | high | critical
category: technical | billing | contract | service | executive
owner: {{owner}}
status: open | investigating | resolved | closed
created: {{date}}
last-modified: {{date}}
author: {{author}}
tags: [escalation, customer-success]
related:
  - "[[]]"
---

# Escalation: {{title}}

## Issue Summary
<!-- What happened, when it started, who reported it -->

## Impact
<!-- Business impact on the customer — revenue, operations, users affected -->

## Root Cause
<!-- Identified root cause or current hypothesis -->

## Resolution Steps
<!-- Actions taken or planned to resolve -->
- [ ]
- [ ]

## Customer Communication
<!-- What has been communicated to the customer, by whom, when -->
-

## Outcome
<!-- Final resolution and customer response -->

## Lessons Learned
<!-- What can we do to prevent this in the future -->
-

## Connections
<!-- Wiki-links to related vault documents -->
- [[]] — related account file
- [[]] — recent interaction notes
- [[]] — matching persona
